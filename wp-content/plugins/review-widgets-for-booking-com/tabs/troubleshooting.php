...
$plugin_updated = ($trustindex_pm_booking->get_plugin_current_version() <= "9.5");
$css_inline = get_option($trustindex_pm_booking->get_option_name('load-css-inline'), 0);
$css = get_option($trustindex_pm_booking->get_option_name('css-content'));
?>
<div class="ti-box">
<div class="ti-header"><?php echo TrustindexPlugin_booking::___("Troubleshooting"); ?></div>
...
<ul>
<li>
<?php echo TrustindexPlugin_booking::___('Use the latest version:') .' '. ($plugin_updated ? $yes_icon : $no_icon); ?>
<?php if(!$plugin_updated): ?>
<a href="/wp-admin/plugins.php"><?php echo TrustindexPlugin_booking::___("Update"); ?></a>
<?php endif; ?>
</li>
<li>
<?php echo TrustindexPlugin_booking::___('Use automatic plugin update:') .' '. (in_array($plugin_slug, $auto_updates) ? $yes_icon : $no_icon); ?>
<?php if(!in_array($plugin_slug, $auto_updates)): ?>
<a href="?page=<?php echo sanitize_text_field($_GET['page']); ?>&tab=troubleshooting&auto_update"><?php echo TrustindexPlugin_booking::___("Enable"); ?></a>
...
<ul>
<li><?php
$upload_dir = dirname($trustindex_pm_booking->getCssFile());
echo TrustindexPlugin_booking::___('writing permission') .' (<strong>'. $upload_dir .'</strong>): '. (is_writable($upload_dir) ? $yes_icon : $no_icon); ?>
</li>
<li>
...
{
echo $no_icon .' '. TrustindexPlugin_booking::___("corrupted") .'
<div class="ti-notice notice-warning">
<p><a href="?page='. sanitize_text_field($_GET['page']) .'&tab=troubleshooting&delete_css">'. TrustindexPlugin_booking::___("Delete the CSS file at <strong>%s</strong>.", [ $trustindex_pm_booking->getCssFile() ]) .'</a></p>
</div>';
}
}
else
...
<li><?php echo TrustindexPlugin_booking::___('Navigate to'); ?> "Settings" > "WP Rocket menu" > "File optimization"</li>
<li><?php echo TrustindexPlugin_booking::___('Scroll to'); ?> "Excluded Javascript files"</li>
...
<?php
$dir = __DIR__ . DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR . 'review-widgets-for-booking-com.php';
...
CSS path: <?php echo esc_html($trustindex_pm_booking->getCssFile()) ."\n\n"; ?>
PHP Info: <?php echo "\n\t"; ?>
...
<a href=".ti-troubleshooting-info" class="btn-text btn-copy2clipboard ti-pull-right"><?php echo TrustindexPlugin_booking::___("Copy to clipboard"); ?></a>
<div class="clear"></div>
</div>
<div class="ti-box">
<div class="ti-header"><?php echo TrustindexPlugin_booking::___("Re-create plugin"); ?></div>
...
<a href="?page=<?php echo esc_attr($_GET['page']); ?>&tab=setup_no_reg&recreate" class="btn-text btn-refresh ti-pull-right" data-loading-text="<?php echo TrustindexPlugin_booking::___("Loading") ;?>" style="margin-left: 0"><?php echo TrustindexPlugin_booking::___("Re-create plugin"); ?></a>
<div class="clear"></div>
</div>
<div class="ti-box">
<div class="ti-header"><?php echo TrustindexPlugin_booking::___("Translation"); ?></div>
<p>
